...
input = (torch.from_numpy(K))[None, None, 42:, 42:].float().cuda() # make the input shape [1, 1, 882, 882] sub dis boundary 42, because input shape must have 4 dimensions.
print(target)
filename = 'loss.dat'
...
Kf = input.squeeze().cpu().numpy()
f = target.cpu().numpy().T
u_exact = np.dot(inv(input.squeeze().cpu().numpy()), target.cpu().numpy().T)
u_predict = net.conv1.weight.detach().cpu().numpy().squeeze()[:, None]
